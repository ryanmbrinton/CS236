...
int main(int argc, char* argv[])
{
	...
	{
		cout << myTokens.at(i)->toString() << endl; // NEW format
	}*/ // commented out for project 2

	DatalogProgram datalogData(myTokens);
	if (datalogData.isSyntaxValid())
	{
		//cout << "Success!" << endl; // commented out for project 3
		datalogData.generateLists();
		//datalogData.toString(); // Commented out for project 3 // doesn't work now, due to change in P3
		//datalogData.printDomain(); // Commented out for project 3
	}

	Database myDatabase(datalogData.getSchemes(), datalogData.getFacts(), datalogData.getRules(),
		datalogData.getQueries());
	myDatabase.populateRelations();
	myDatabase.answerQueries();
...
unsigned int s0(vector<string>& inputStrings, vector<TokenData*>& myTokens, unsigned int& lineIter)
{
	...
	case '\'':
		...
		/*myTokens.push_back(new TokenData(STRING, "FIXME", lineIter + 1));
		++charIter;
		inputStrings.at(lineIter).erase(0, charIter);
		charIter = 0; //  will be a string*/
		break;
	case '#':
		++charIter;
