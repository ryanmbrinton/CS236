	...
	void generateLists()
	{
		...
		while (j < myTokens.size()) // populate schemes
		{
			...
			else
			{
				...
				{
					mySchemes.push_back(new Predicate(SCHEMES, firstID, schemeStrings));
					schemeStrings.clear();
					firstID = "";
				}
			}
		...
		while (j < myTokens.size()) // populate facts
		{
			...
			else
			{
				...
				{
					myFacts.push_back(new Predicate(FACTS, firstID, factStrings));
					factStrings.clear();
					firstID = "";
					getLeadID = true;
				}
			...
			else
			{
				...
				else if (myTokens.at(j)->getTokenType() == RIGHT_PAREN)
				{
					...
					if (parCounter == 0)
					{
						...
						{
							++j;
							myQueries.push_back(new Predicate(QUERIES, firstID, queryStrings));
							queryStrings.clear();
						}
						getLeadID = true;
					}
