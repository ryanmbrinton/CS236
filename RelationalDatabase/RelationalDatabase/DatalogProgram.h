	...
	void generateLists()
	{
		...
		while (j < myTokens.size()) // populate schemes
		{
			if (getLeadID)
			{
				...
				{
					firstID = myTokens.at(j)->getString();
					getLeadID = false;
				}
			}
			else
			{
				...
				if (myTokens.at(j)->getTokenType() == RIGHT_PAREN)
				{
					...
					mySchemes.push_back(Predicate(SCHEMES, firstID, schemeStrings));
					getLeadID = true;
					schemeStrings.clear();
					firstID = "";
				}
		...
		while (j < myTokens.size()) // populate facts
		{
			...
			else
			{
				...
				{
					myFacts.push_back(Predicate(FACTS, firstID, factStrings));
					factStrings.clear();
					firstID = "";
					getLeadID = true;
				}
		...
		while (j < myTokens.size()) // populate rules
		{
			...
			else
			{
				...
				{
					myRules.push_back(Rule(ruleStringsBefore, ruleStringsAfter));
		...
		{
			//cout << myTokens.at(j)->getString() << endl;
			if (getLeadID)
			{
				if (myTokens.at(j)->getTokenType() == ID)
				{
			...
			else
			{
				...
				else if (myTokens.at(j)->getTokenType() == RIGHT_PAREN)
				{
					...
					if (parCounter == 0)
					{
						...
						{
							++j;
							myQueries.push_back(Predicate(QUERIES, firstID, queryStrings));
							queryStrings.clear();
							firstID = "";
						}
						getLeadID = true;
				...
				else if (myTokens.at(j)->getTokenType() == STRING)
				{
					queryStrings.push_back(myTokens.at(j)->getString());
				}
				else
				{
					//cout << myTokens.at(j)->getString();
					//dataOSS << myTokens.at(j)->getString();
				}
			}
			++j;
